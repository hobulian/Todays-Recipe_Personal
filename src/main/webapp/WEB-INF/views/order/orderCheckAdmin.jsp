...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
 ...
 $(function() { 
		...
		console.log("배송완료버튼 클릭 ");
		var num= $(this).attr("data-num");
		var status = $("#receiptstatus" + num).text();
		var xxx= $(this);
		console.log(num);
		console.log(status);
		if(confirm("상품을 배송완료처리 하시겠습니까?"))  //확인을 누르면 true, 취소 누르면 false
		{
		$.ajax({
			url: "receiptStatusAdmin",
			type:"get",
			dataType: "text",
			data: {
				num: num,
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <table class="table table-hover">
                        ...
                        <c:forEach var="getOrderList" items="${OrderList}">
                            <tr>
                                ...
                                <c:if test="${ getOrderList.status eq '주문접수' }">
                                	<button type="button" class="btn btn-info confirmgoodsreceipt"  data-num="${getOrderList.num}">배송완료</button>
                                </c:if>
                                </div> 
                                </td>
                            <%--     <td>${getOrderList.gAmount}</td>
